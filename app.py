...
def comprehensive_eda(df):
    ...
    with col2:
        ...
        buffer = []
        buffer.append("Kolom | Tipe | Non-Null | Unique")
        buffer.append("---|---|---|---")
        for col in df.columns:
            dtype = str(df[col].dtype)
            non_null = df[col].count()
            unique = df[col].nunique()
            buffer.append(f"{col} | {dtype} | {non_null} | {unique}")
        st.markdown("\n".join(buffer))
...
def preprocessing_analysis(df):
    ...
    if 'NObeyesdad' in df_encoded.columns:
        ...
        target_mapping = {orig: encoded for orig, encoded in zip(original_target, 
...
def standardization_analysis(X, feature_names):
    ...
    X.iloc[:, :5].boxplot(ax=ax1)  # Tampilkan 5 fitur pertama
    ax1.set_title('Distribusi Sebelum Standardisasi')
    ax1.set_ylabel('Nilai')
    plt.setp(ax1.get_xticklabels(), rotation=45)
    
    # Setelah standardisasi
    X_scaled.iloc[:, :5].boxplot(ax=ax2)  # Tampilkan 5 fitur pertama
    ...
    for name, model in models_dict.items():
        try:
            ...
            try:
                ...
                roc_auc = 0  # Jika model tidak support predict_proba
            ...
            results['CV Score Mean'].append(cv_scores.mean())
            results['CV Score Std'].append(cv_scores.std())
...
def main():
    ...
    if selected_section == "📊 EDA (Exploratory Data Analysis)":
        comprehensive_eda(df)
    
    elif selected_section == "🔄 Preprocessing Data":
        df_processed, encoders, target_encoder = preprocessing_analysis(df)
        # Simpan hasil preprocessing ke session state
        st.session_state['df_processed'] = df_processed
        st.session_state['encoders'] = encoders
        st.session_state['target_encoder'] = target_encoder
    
    elif selected_section == "⚖️ SMOTE & Balancing":
        if 'df_processed' not in st.session_state:
            st.warning("⚠️ Silakan jalankan preprocessing terlebih dahulu!")
            return
        
        df_processed = st.session_state['df_processed']
        X_res, y_res = smote_analysis(df_processed)
        
        # Standardisasi
        X_scaled, scaler = standardization_analysis(X_res, df_processed.drop('NObeyesdad', axis=1).columns)
    ...
    elif selected_section == "🚀 Baseline Models":
        # Cek apakah data sudah diproses
        required_keys = ['X_train', 'X_test', 'y_train', 'y_test']
        if not all(key in st.session_state for key in required_keys):
            st.warning("⚠️ Silakan jalankan preprocessing dan SMOTE terlebih dahulu!")
            return
        ...
        with st.spinner("Menjalankan evaluasi baseline models..."):
            baseline_results = evaluate_models_comprehensive(
                baseline_models, X_train, X_test, y_train, y_test, 'Baseline'
            )
        
        # Tampilkan hasil
        st.markdown("### 📊 Hasil Baseline Models")
        st.dataframe(baseline_results.round(4))
        
        # Visualisasi perbandingan
        st.markdown("### 📈 Visualisasi Performa Baseline Models")
        ...
        for i, metric in enumerate(metrics_to_plot):
            ax = axes[i]
            baseline_results.plot(x='Model', y=metric, kind='bar', ax=ax, color='skyblue')
            ax.set_title(f'{metric} - Baseline Models')
    ...
    elif selected_section == "🔧 Hyperparameter Tuning":
        # Cek apakah baseline models sudah dijalankan
        if 'baseline_models' not in st.session_state:
            st.warning("⚠️ Silakan jalankan baseline models terlebih dahulu!")
            return
